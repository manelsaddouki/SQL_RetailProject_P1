...
select years, months, max(avg_sale)
from(
	select 
		to_char(sale_date, 'YYYY')as years,
		to_char(sale_date, 'YYYY-MM')as months, 
...
select category, 
	count(distinct(customer_id)) as cst
from retail_sales
group by category;
...
from (SELECT
    ...
    CASE
        ...
        ELSE 'Evening'
    END AS shift, 
	count(distinct(transactions_id)) as ct_trs
FROM retail_sales
group by sale_time) as t
group by shift;
